...
#import "InsightsFeedbackWidget.h"
...
/**
 * Returns @c Insights singleton to be used throughout the app.
 * @return The shared @c Insights object
 */
+ (instancetype)sharedInstance;

/**
 * Starts Insights with given configuration and begins session.
 * @param config @c InsightsConfig object that defines host, app key, optional features and other settings
 */
...
#if (TARGET_OS_WATCH)
/**
 * Suspends Insights, adds recorded events to request queue and ends current session.
 * @discussion This method needs to be called manually only on @c watchOS, on other platforms it will be called automatically.
 */
- (void)suspend;

/**
 * Resumes Insights, begins a new session after the app comes to foreground.
 * @discussion This method needs to be called manually only on @c watchOS, on other platforms it will be called automatically.
 */
- (void)resume;
#endif
 ...
 * @discussion @c INSDeviceIDTypeCustom : Custom device ID set by app developer.
 * @discussion @c INSDeviceIDTypeTemporary : Temporary device ID. See @c INSTemporaryDeviceID for details.
 * @discussion @c INSDeviceIDTypeIDFV : Default device ID type used by the SDK on iOS and tvOS.
 * @discussion @c INSDeviceIDTypeNSUUID  : Default device ID type used by the SDK on watchOS and macOS.
 */
- (INSDeviceIDType)deviceIDType;
 ...
 * @discussion When passing @c INSTemporaryDeviceID for @c deviceID parameter, argument for @c onServer parameter does not matter.
 * @discussion When setting a new device ID while the current device ID is @c INSTemporaryDeviceID, argument for @c onServer parameter does not matter.
 * @param deviceID New device ID
 * @param onServer If set, data on Insights Server will be merged automatically, otherwise device will be counted as a new device
 */
 ...
 */
- (void)giveConsentForFeature:(INSConsent)featureName;
 ...
 */
- (void)giveConsentForFeatures:(NSArray<INSConsent> *)features;
 ...
 */
- (void)cancelConsentForFeature:(INSConsent)featureName;
 ...
 */
- (void)cancelConsentForFeatures:(NSArray<INSConsent> *)features;
 ...
 * @discussion Custom objects in segmentation will cause events not to be sent to Insights Server.
 * @discussion Nested values in segmentation will be ignored by Insights Server event segmentation section.
 * @param key Event key, a non-zero length valid string
 ...
 * @discussion Logs recorded by this method are stored in a non-persistent structure, and delivered to Insights Server only in case of a crash.
 ...
 * @discussion If segmentation has any of Insights reserved keys, they will be ignored:
...
/**
 * Returns @c InsightsUserDetails singleton to be used throughout the app.
 * @return The shared @c InsightsUserDetails object
 */
 ...
 * @discussion If user dismissed dialog without giving a rating, this value will be 0 and it will not be sent to Insights Server.
 ...
 * @discussion When feedback widgets are fetched successfully, @c completionHandler will be executed with an array of @c InsightsFeedbackWidget objects.
 * @discussion Otherwise, @c completionHandler will be executed with an @c NSError.
 * @discussion Calls to this method will be ignored and @c completionHandler will not be executed if:
 * @discussion - Consent for @c INSConsentFeedback is not given, while @c requiresConsent flag is set on initial configuration.
 * @discussion - Current device ID is @c INSTemporaryDeviceID.
 ...
 */
- (void)getFeedbackWidgets:(void (^)(NSArray <InsightsFeedbackWidget *> * __nullable feedbackWidgets, NSError * __nullable error))completionHandler;
 ...
 * @discussion If remote config is never retrieved from Insights Server before, this method will return @c nil.
 * @discussion If @c key is not defined in remote config on Insights Server, this method will return @c nil.
 * @discussion If Insights Server is not reachable, this method will return the last retrieved value which is stored on device.
 * @param key Remote config key specified on Insights Server
 */
- (id)remoteConfigValueForKey:(NSString *)key;

/**
 * Manually updates all locally stored remote config values by fetching latest values from Insights Server, and executes completion handler.
...
/**
 * Manually updates locally stored remote config values only for specified keys, by fetching latest values from Insights Server, and executes completion handler.
 * @discussion @c completionHandler has an @c NSError parameter that will be either @ nil or an @c NSError object, depending on result.
 * @discussion Calls to this method will be ignored and @c completionHandler will not be executed if:
 * @discussion - There is not any consent given, while @c requiresConsent flag is set on initial configuration.
 * @discussion - Current device ID is @c INSTemporaryDeviceID.
...
/**
 * Manually updates locally stored remote config values except for specified keys, by fetching latest values from Insights Server, and executes completion handler.
 * @discussion @c completionHandler has an @c NSError parameter that will be either @ nil or an @c NSError object, depending on result.
 * @discussion Calls to this method will be ignored and @c completionHandler will not be executed if:
 * @discussion - There is not any consent given, while @c requiresConsent flag is set on initial configuration.
 * @discussion - Current device ID is @c INSTemporaryDeviceID.
 ...
 * @discussion - Consent for @c INSConsentPerformanceMonitoring is cancelled
